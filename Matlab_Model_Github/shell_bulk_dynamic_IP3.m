function dS = shell_bulk_dynamic_IP3(t,S,ATP)

f = 0.01; % buffering
%% Diffusion between Compartments
[J_diff_c, J_diff_p] = dif(S(1),S(4),S(2),S(5));
...
%% Shell
% Calcium 
J_IPR_s = JIPR(S(3),S(1),S(7),S(2));
J_leak_s = JERLEAK(S(1),S(7));
J_SERCA_s = JSERCA(S(1));
...
% Calcium
J_IPR_b = JIPR(S(6),S(4),S(7),S(5));
J_leak_b = JERLEAK(S(4),S(7));
J_SERCA_b = JSERCA(S(4));
